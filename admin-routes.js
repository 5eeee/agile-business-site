...
const DEFAULT_CALCULATOR_PRICING_ADMIN = require('./data/default-calculator-pricing.json');
...
module.exports = function createAdminRouter(pool, hooks = {}) {
    ...
    router.get('/calculator/pricing', requireAdmin, async (req, res) => {
        try {
            ...
            const pricing = raw ? JSON.parse(raw) : DEFAULT_CALCULATOR_PRICING_ADMIN;
    ...
    router.put('/calculator/pricing', requireRole('admin', 'director'), async (req, res) => {
        try {
            ...
            clearPublicCaches(['pricing:calculator']);
            res.json({ ok: true });
        } catch (e) {
            res.status(500).json({ error: 'Ошибка сервера' });
        }
